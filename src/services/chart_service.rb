...
class ChartService
  ...
  def generate_data(year, season)
    ...
    {test: 'value', debug: dedupped_data[0].to_a.to_h, ratings: ratings, last_season_comparisons: last_seasons_comparisons, hof: hof}
  end
  ...
  def calculate_ratings_data(csv_headers, dedupped_data)
    ...
    dedupped_data.each do |row|
      indexes.each do |face_code, index|
        ...
        # face_scores[face_code][:total] += 1
        # face_scores[face_code][:ratings][score] += 1
        face_scores[face_code][score] += 1
      end
    end

    face_scores = face_scores.map do |face_code, hash|
      [face_code, calculate_stats(hash)]
    end.to_h
  ...
  def calculate_stats(ratings)
    ...
    ratings.each do |(score, count)|
      total += count
      avg += score * count
      baseballs_top_weighted += BASEBALLS_TOP_WEIGHTED_WEIGHTS[score] * count
    end
    avg /= total.to_f
    baseballs_top_weighted *= BASEBALLS_TOP_WEIGHTED_MULTIPLIER
    baseballs_top_weighted /= total
    {responses: total, avg: avg, baseballsTopWeighted: baseballs_top_weighted, ratings: ratings}
  end
